...
This option controls the big integer class into which the objects will be
blessed.  The default is Math::BigInt.
...
converted into a big integer object.

=item raw

If this option is specified, size in not checked, there is no conversion
and the integer is treated as if it is already encoded.
...
While decoding, this options controls how integers are handled.  Math::BigInt
is the default.  Either the name of a class that can handle big integers or
raw may be specified.
...
If this option is specified, no conversion is performed and the encoded
integer is passed as is regardless of size.
